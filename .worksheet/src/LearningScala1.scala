object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(126); 
  ...
  println("Here is a mess: " + numberOne + truth + letterA + pi + bigNumber);$skip(149); 
  
  // printf style: (.3f means floating point value with 3 decimal points of precision on the RHS
  ...
  val isBest: Boolean = picard == bestCaptain;System.out.println("""isBest  : Boolean = """ + $show(isBest ));$skip(315); 
  ...
  println(piSinglePrecision * 2);$skip(39); 
  val doublePi = piSinglePrecision * 2;System.out.println("""doublePi  : Float = """ + $show(doublePi ));$skip(18); 
  print(doublePi);$skip(150); 
  println(f"This is chooryfury trying to code in Scala. She has aced and mastered the art of Scala coding. Hooray! And the asnwer is: $doublePi%.3f")}
